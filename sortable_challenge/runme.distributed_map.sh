...
for i in $(seq 2 5);do
  erl -pa ebin -pa ../distributed_map/ebin -noinput -setcookie crap -sname node${i}@localhost &
...
echo "===================="
erl -pa ebin -pa ../distributed_map/ebin -setcookie crap -sname node1@localhost -run distmap setupcluster ${NODELIST}

echo "Killing Erlang worker nodes"
for PID in $(ps axu | grep "\-noinput \-setcookie crap" | grep -v "grep" | sed "s/^[^\ ]*\ *\([0-9]*\)\ *.*/\1/g"); do
  kill -HUP ${PID}
done
